...
const Notification = () => {
  ...
  const [smsActivity, setSmsActivity] = useState(false);
      ...
      <div className="flex flex-grow">
        ...
        <div className="w-[100px] p-4">
          <div className="h-[434px] px-1 py-6 bg-orange-200 rounded-lg shadow-[0px_4px_6px_0px_rgba(0,0,0,0.09)] flex-col justify-center items-center gap-7 inline-flex">
            ...
            {/* Analysis Button */}
            <Link to="/analysis" className="h-[49px] p-1 bg-transparent hover:bg-orange-300 rounded-md flex flex-col justify-center items-center gap-2">
              <div data-svg-wrapper className="relative">
            ...
            <Link to="/notifications" className="gap-2 p-1 bg-[#ff904f]  hover:bg-orange-300 rounded-md flex flex-col justify-center items-center">
              <div data-svg-wrapper className="relative">
                <svg width="25" height="24" viewBox="0 0 25 24" fill="none" xmlns="http://www.w3.org/2000/svg">
              ...
              <div className="text-center text-[#FFE3D3] text-xs font-normal font-['Roboto'] leading-none">Notification</div>
            </Link>

            {/* Settings Button */}
            <Link to="/profile" className="h-[49px] flex flex-col justify-center items-center rounded-md px-3 py-2 hover:bg-orange-300 gap-2">
              <div data-svg-wrapper className='relative'>
                <svg width="25" height="24" viewBox="0 0 25 24" fill="none" xmlns="http://www.w3.org/2000/svg">
              ...
              <div className="text-[#393939] text-xs font-extrabold font-['Roboto'] leading-none">Setting</div>
            </Link>
          </div>
        </div>
        ...
        <div className="flex-1 flex flex-col items-center mx-auto">
          <div className="p-5 w-full max-w-6xl">
            ...
            <div className="mt-8 w-[775px] mx-auto text-center">
              <h3 className="text-2xl font-semibold text-[#393939]">Notification</h3>
              <p className="text-gray-600 mb-6">Manage your notification preferences here.</p>

              {/* Reminders Section */}
              <div className="bg-[#fffff] rounded-[20px] p-6 mb-6 text-left">
                <h4 className="text-xl font-semibold text-[#393939] mb-3">Reminders</h4>
                <div className="flex justify-between items-start gap-6">
                  <p className="text-gray-600 flex-1">
                    Trace on notifications to remind you of remind you of holds you might have missed.
            </p>
            {/* Toggle Buttons (Stacked) */}
            <div className="flex flex-col gap-2">
              <div className="flex items-center gap-3">
                <button
                  className={`w-[65px] h-7 rounded-full p-1 flex items-center transition-all duration-300 ${
                    emailReminder ? "bg-[#fd8129]" : "bg-[#e0ecf4]"
                  }`}
                  onClick={() => setEmailReminder(!emailReminder)}
                >
                  <div
                    className={`w-6 h-6 bg-white rounded-full shadow-md transform transition-all duration-300 ${
                      emailReminder ? "translate-x-8" : "translate-x-0"
                    }`}
                  ></div>
                </button>
                <span className="text-black text-lg font-medium">Email</span>
              </div>
              <div className="flex items-center gap-3">
                <button
                  className={`w-[65px] h-7 rounded-full p-1 flex items-center transition-all duration-300 ${
                    smsReminder ? "bg-[#fd8129]" : "bg-[#e0ecf4]"
                  }`}
                  onClick={() => setSmsReminder(!smsReminder)}
                >
                  <div
                    className={`w-6 h-6 bg-white rounded-full shadow-md transform transition-all duration-300 ${
                      smsReminder ? "translate-x-8" : "translate-x-0"
                    }`}
                  ></div>
                </button>
                <span className="text-black text-lg font-medium">SMS</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="bg-[#fffff] rounded-[20px] p-6 text-left">
          <h4 className="text-xl font-semibold text-[#393939] mb-3">More Activity</h4>
          <div className="flex justify-between items-start gap-6">
            <p className="text-gray-600 flex-1">
              Trace might be new suggestions held for you.
            </p>
            {/* Toggle Buttons (Stacked) */}
            <div className="flex flex-col gap-2">
              <div className="flex items-center gap-3">
                <button
                  className={`w-[65px] h-7 rounded-full p-1 flex items-center transition-all duration-300 ${
                    emailActivity ? "bg-[#fd8129]" : "bg-[#e0ecf4]"
                  }`}
                  onClick={() => setEmailActivity(!emailActivity)}
                >
                  <div
                    className={`w-6 h-6 bg-white rounded-full shadow-md transform transition-all duration-300 ${
                      emailActivity ? "translate-x-8" : "translate-x-0"
                    }`}
                  ></div>
                </button>
                <span className="text-black text-lg font-medium">Email</span>
              ...
              <div className="flex items-center gap-3">
                <button
                  className={`w-[65px] h-7 rounded-full p-1 flex items-center transition-all duration-300 ${
                    smsActivity ? "bg-[#fd8129]" : "bg-[#e0ecf4]"
                  }`}
                  onClick={() => setSmsActivity(!smsActivity)}
                >
                  <div
                    className={`w-6 h-6 bg-white rounded-full shadow-md transform transition-all duration-300 ${
                      smsActivity ? "translate-x-8" : "translate-x-0"
                    }`}
                  ></div>
                </button>
                <span className="text-black text-lg font-medium">SMS</span>
